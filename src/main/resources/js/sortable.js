...
// Modified by John Farrell, Dec 2016
//   * used jQuery for things jQuery can do.
//   * integrated shaded table stuff

$(window).on("load", sortables_init);
$(window).on("load", shadedtable_init);
...
function shadedtable_init() {
    // Find all tables with class shaded and make them shaded
    if (!document.getElementsByTagName) return;
    var tbls = document.getElementsByTagName("table");
    for (var ti=0;ti<tbls.length;ti++) {
        if ($(tbls[ti]).hasClass("shaded")) make_shaded($(tbls[ti]));
    }
}

function make_shaded(table) {
    var rows = table.find('tr');
    rows.each(function(index, row) {
        $(row).removeClass("even");
    });
    for (var i=0; i<rows.length; i+=2) $(rows[i]).addClass("even");
}
...
function sortables_init() {
    ...
    var tbls = document.getElementsByTagName("table");
    for (var ti=0;ti<tbls.length;ti++) {
        if ( $(tbls[ti]).hasClass( "sortable" ) && (tbls[ti].id)) {
...
function ts_resortTable(lnk) {
    ...
    for (i=0; i<table.tBodies[0].rows.length; i++) {
        if (!$( table.tBodies[0].rows[i]).hasClass( "sectionheader" ) && ts_getInnerText(table.tBodies[0].rows[i].cells[column]) != "") {
            test_row = i;
            break;
        }
    }
    ...
    for (j=0;j<table.tBodies[0].rows.length;j++) {
        ...
        if (! $( table.tBodies[0].rows[j]).hasClass( "sectionheader" )) {
    ...
    for (i=0;i<newRows.length;i++) { if ( !$( newRows[i] ).hasClass( 'sortbottom' ) ) table.tBodies[0].appendChild(newRows[i]);}
    ...
    if ($(table).hasClass( "shaded" ) ) make_shaded($(table));
}
